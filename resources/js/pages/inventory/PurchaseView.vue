...
<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        ...
        <div class="space-y-6 p-4 md:p-6">
            ...
            <!-- Error State -->
            <div v-else-if="error" class="bg-destructive/10 border border-destructive/40 rounded-lg p-4">
            ...
            <div v-else-if="purchase" class="space-y-6">
                ...
                <div class="bg-card rounded-lg border p-4 md:p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                        <div>
                            <label class="text-sm font-medium text-muted-foreground">Purchase Date</label>
                    ...
                    <div v-if="purchase.note">
                        ...
                        <p class="text-foreground">{{ purchase.note }}</p>
                    </div>
                </div>
                        ...
                        <table class="min-w-full divide-y divide-border">
                            <thead class="bg-muted">
                                <tr>
                                    ...
                                    <th class="px-3 md:px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Quantity</th>
                                    <th class="px-3 md:px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Purchase Rate</th>
                                    <th class="px-3 md:px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Sale Rate</th>
                                    <th class="px-3 md:px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Total</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-border bg-card">
                                <tr v-for="item in purchase.items" :key="item.id" class="hover:bg-accent">
                            ...
                            <tfoot v-if="purchase.items && purchase.items.length > 0" class="bg-muted">
                                <tr>
                                    <td colspan="4" class="px-3 md:px-6 py-4 text-sm font-bold text-foreground text-right">
                                        Grand Total:
                                    </td>
                                    <td class="px-3 md:px-6 py-4 text-lg font-bold text-success">
                                        {{ formatCurrency(purchase.total_amount) }}
                                    </td>
                                </tr>
                            </tfoot>

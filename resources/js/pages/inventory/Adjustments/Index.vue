    ...
    <AppLayout :breadcrumbs="breadcrumbItems">
        ...
        <div class="space-y-6 p-4 md:p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold text-foreground">Stock Adjustments</h1>
                    <p class="text-muted-foreground">Track and manage stock corrections</p>
                </div>
                <Link
                    :href="`/inventory/adjustments/create`"
                    class="px-4 py-2 bg-success text-success-foreground rounded-lg hover:bg-success/90 transition-colors min-h-11 flex items-center justify-center"
                >
                    New Adjustment
                </Link>
            </div>
            ...
            <div class="bg-card rounded-lg shadow-sm p-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-muted-foreground mb-1">Item</label>
                        <select
            ...
            <div class="table-scroll bg-card rounded-lg shadow-sm">
                    ...
                    <tbody class="bg-card divide-y divide-border">
                        <tr v-for="adjustment in props.adjustments.data" :key="adjustment.id" class="hover:bg-accent">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <span v-if="adjustment.type === 'add'" class="text-success font-medium">
                                    +{{ adjustment.quantity }}
                                </span>
                                <span v-else-if="adjustment.type === 'subtract'" class="text-destructive font-medium">
                                    -{{ adjustment.quantity }}
                                </span>
                ...
                <div v-if="props.adjustments.data.length === 0" class="px-6 py-12 text-center">
                    <p class="text-muted-foreground">No adjustments found.</p>
                </div>

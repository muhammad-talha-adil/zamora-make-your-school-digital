        ...
        <div class="space-y-6 p-4 md:p-6">
            ...
            <div class="bg-card rounded-lg border border-border p-4 md:p-6">
                <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-base font-semibold text-foreground">{{ structure.title ?? 'N/A' }}</p>
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-base text-foreground">{{ structure.session?.name || 'N/A' }}</p>
                    </div>

                    <!-- Campus -->
                    <div class="space-y-1">
                        <p class="text-sm font-medium text-muted-foreground">Campus</p>
                        <p class="text-base text-foreground">{{ structure.campus?.name || 'N/A' }}</p>
                    </div>

                    <!-- Class -->
                    <div class="space-y-1">
                        <p class="text-sm font-medium text-muted-foreground">Class</p>
                        <p class="text-base text-foreground">{{ structure.class?.name || 'All Classes' }}</p>
                    </div>

                    <!-- Section -->
                    <div class="space-y-1">
                        <p class="text-sm font-medium text-muted-foreground">Section</p>
                        <p class="text-base text-foreground">{{ structure.section?.name || 'All Sections' }}</p>
                    </div>

                    <!-- Effective From -->
                    <div class="space-y-1">
                        <p class="text-sm font-medium text-muted-foreground">Effective From</p>
                        <p class="text-base text-foreground">{{ formatDate(structure.effective_from) }}</p>
                    </div>

                    <!-- Effective To -->
                    <div class="space-y-1">
                        <p class="text-sm font-medium text-muted-foreground">Effective To</p>
                        <p class="text-base text-foreground">{{ structure.effective_to ? formatDate(structure.effective_to) : 'Indefinite' }}</p>
                    </div>

                    <!-- Created By -->
                    <div v-if="structure.created_by" class="space-y-1">
                        <p class="text-sm font-medium text-muted-foreground">Created By</p>
                        <p class="text-base text-foreground">{{ structure.created_by.name }}</p>
                    </div>
                </div>

                <!-- Notes -->
                <div v-if="structure.notes" class="mt-6 pt-6 border-t border-border">
                    <p class="text-sm font-medium text-muted-foreground mb-2">Notes</p>
                    <p class="text-base text-foreground whitespace-pre-wrap">{{ structure.notes }}</p>
                </div>
            </div>
            ...
            <div class="bg-card rounded-lg border border-border p-4 md:p-6">
                <div class="flex flex-wrap gap-2 justify-between items-center mb-4">
                    ...
                    <span class="text-sm text-muted-foreground">Total: {{ formatCurrency(totalAmount) }}</span>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-border">
                        ...
                        <tbody class="divide-y divide-border bg-card">
                            <tr v-for="item in structure.items" :key="item.id" class="transition-colors hover:bg-accent">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="text-sm text-muted-foreground">{{ item.type || 'N/A' }}</div>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="text-sm text-muted-foreground">{{ item.frequency || 'N/A' }}</div>
                                </td>
                        ...
                        <tfoot class="bg-muted">
                            <tr>
                                <td colspan="3" class="px-4 py-3 text-sm font-semibold text-foreground text-right">
                                    Total
                                </td>

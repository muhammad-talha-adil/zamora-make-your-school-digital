<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        ...
        <div class="space-y-6 p-4 md:p-6">
            <!-- Header -->
            <div
                class="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center md:gap-4"
            >
                <div>
                    <h1
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <!-- B-Form/CNIC with Auto-formatting -->
                        <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            ...
                            >
                                <option
                                    v-for="status in props.studentStatuses"
                                    :key="status.id"
                                    :value="status.id"
                                    :selected="status.name === 'Active'"
                                >
                                    {{ status.name }}
                                </option>
                            </select>
...
const activeStatusId = computed(() => {
    const active = props.studentStatuses.find(
...
// Sibling lookup when father phone is entered
const lookupFatherGuardian = async () => {
    ...
    if (rawPhone.length < 11) {
        // Clear sibling match if phone is incomplete
    ...
    // Don't lookup if form is processing (prevent API call on submit)
    ...
    try {
        ...
        if (response.data.found) {
            ...
            // Store the linked guardian ID
            linkedGuardianId.value = guardian.id;
            // Auto-populate father information
...
const submitForm = () => {
    ...
    formData.set(
        'gender_id',
        String(parseInt(form.value.gender_id.toString()) || 0),
    );
    formData.set(
        'campus_id',
        String(parseInt(form.value.campus_id.toString()) || 0),
    );
    formData.set(
        'session_id',
        String(parseInt(form.value.session_id.toString()) || 0),
    );
    formData.set(
        'class_id',
        String(parseInt(form.value.class_id.toString()) || 0),
    );
    ...
    formData.set(
        'student_status_id',
        String(
            parseInt(form.value.student_status_id.toString()) ||
    ...
    if (includeOtherGuardian.value && form.value.other_name && form.value.other_name.trim()) {
        ...
        if (form.value.other_relation_id) {
            formData.set(
                'other_relation_id',
                String(parseInt(form.value.other_relation_id.toString())),
            );
        }
        // Only send other phone if provided
    ...
    router.post(route('students.store'), formData, {
        ...
        onError: (err) => {
            ...
            if (errorMessages.length > 0) {
                ...
                alert.error('Failed to admit student. Please check the errors and try again.');
            }
        },
    });
};
